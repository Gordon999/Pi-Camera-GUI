...
text(0,6,2,1,"Please Wait, checking camera",int(fv* 1.7),1)
...
if os.path.exists('test.jpg'):
   ...
   max_speed = 6000000
if Pi_Cam > 0:
    text(0,6,2,1,"Found Pi Camera v" + str(Pi_Cam),int(fv*1.7),1)
else:
    text(0,6,2,1,"No Pi Camera found",int(fv*1.7),1)
...
pygame.display.update()
time.sleep(1)
text(0,6,2,1,"Please Wait for preview...",int(fv*1.7),1)
       ...
       if event.type == QUIT:
           ...
           if mousex > cwidth:
               ...
               if g == 22 or g ==23:
                   ...
                   text(0,6,2,1,"Please Wait, taking still ...",int(fv*1.7),1)
                   ...
                   button(0,0,1)
                   text(0,2,0,1,"CAPTURE",ft,0)
                   text(0,6,2,1,"Please Wait, taking video ...",int(fv*1.7),1)
                   now = datetime.datetime.now()
                   timestamp = now.strftime("%y%m%d%H%M%S")
           ...
           with open('PiCconfig5.txt', 'w') as f:
               for item in config:
                   f.write("%s\n" % item)
           text(0,6,2,1,"Waiting for preview ...",int(fv*1.7),1)
           preview()
