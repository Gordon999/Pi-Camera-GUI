...
frame       = 0       # set to 1 for no frame (ie if using Pi 7" touchscreen)
...
config_file = "/home/pi/PiConfig4.txt"
...
still_limits = [mode,0,len(modes)-1,speed,0,len(speeds)-1,ISO,0,800,brightness,0,100,contrast,-100,100,ev,-12,12,blue,0.1,4,red,0.1,8,extn,0,len(extns)-1,effect,0,len(effects)-1,meter,0,len(meters)-1,awb,0,len(awbs)-1]
...
if not os.path.exists(config_file):
    points = [mode,speed,ISO,brightness,contrast,frame,int(red*10),int(blue*10),ev,vlen,fps,vformat,a_video,tinterval,tshots,tduration,extn,zx,zy,zoom,effect,meter,awb]
...
def draw_bar(col,row,color,value):
    global bw,bh,preview_width,still_limits
...
def preview():
    global p, brightness,contrast,modes,mode,red,blue,ISO,sspeed,ev,preview_width,preview_height,zoom,igw,igh,zx,zy
    speed2 = sspeed
    if speed2 > 6000000:
        speed2 = 6000000
...
text(1,11,2,0,1,"Save Config",fv,7)
...
text(1,12,2,0,1,"EXIT",fv,7)

# draw sliders
for k in range(3,len(still_limits)-12,3):
...
while True:
    ...
    if buttonx[0] != 0 :
        ...
        if mousex > preview_width:
            x = int((mousex-preview_width)/int(bw/2))
            y = int((mousey)/bh)
            g = (y*4) + x
            if g == 9 or g == 8:
                if mode == 0 :
                    if mousey < (y*bh) + 10:
                        speed = int(((mousex-preview_width) / bw) * (still_limits[((y-1)*3) + 2] - still_limits[((y-1)*3) + 1]))
                        shutter = speeds[speed]
                        if shutter < 0:
                            shutter = abs(1/shutter)
                        sspeed = int(shutter * 1000000)
                ...
                if mousey < (y*bh) + 10:
                    ISO = int(((mousex-preview_width) / bw) * (still_limits[((y-1)*3) + 2] - still_limits[((y-1)*3) + 1]))
                    if ISO < 100:
                        ISO = 0
                    elif ISO < 200:
                        ISO = 100
                ...
                if mousey < (y*bh) + 10:
                    brightness = int(((mousex-preview_width) / bw) * (still_limits[((y-1)*3) + 2] - still_limits[((y-1)*3) + 1])) 
                elif mousey < (y*bh) + (bh/1.2):
                    if mousex > preview_width + (bw/2):
                        brightness +=1
                ...
                if mousey < (y*bh) + 10:
                    contrast = int(((mousex-preview_width) / bw) * (still_limits[((y-1)*3) + 2] - still_limits[((y-1)*3) + 1])) - 100
                elif mousey < (y*bh) + (bh/1.2):
                    if mousex > preview_width + (bw/2):
                        contrast +=1
                ...
                if mousey < (y*bh) + 10:
                    ev = int(((mousex-preview_width) / bw) * (still_limits[((y-1)*3) + 2] - still_limits[((y-1)*3) + 1])) - 12
                elif mousey < (y*bh) + (bh/1.2):
                    if mousex > preview_width + (bw/2):
                        ev +=1
                ...
                if mousey < (y*bh) + 10:
                    vlen = int(((mousex-preview_width - bw) / bw) * (video_limits[((y-1)*3) + 2] - video_limits[((y-1)*3) + 1]))
                elif mousey < (y*bh) + (bh/1.2):
                ...
                if mousey < (y*bh) + 10:
                    tinterval = int(((mousex-preview_width-bw) / bw) * (video_limits[((y-1)*3) + 2] - video_limits[((y-1)*3) + 1]))
                elif mousey < (y*bh) + (bh/1.2):
                    if mousex > preview_width + bw + (bw/2):
                        tinterval +=1
                ...
                if mousey < (y*bh) + 10:
                    tduration = int(((mousex-preview_width - bw) / bw) * (video_limits[((y-1)*3) + 2] - video_limits[((y-1)*3) + 1])) 
                elif mousey < (y*bh) + (bh/1.2):
                    if mousex > preview_width + bw + (bw/2):
                        tduration +=1
                ...
                if mousey < (y*bh) + 10:
                    tshots = int(((mousex-preview_width-bw) / bw) * (video_limits[((y-1)*3) + 2] - video_limits[((y-1)*3) + 1]))
                elif mousey < (y*bh) + (bh/1.2):
                    if mousex > preview_width + bw + (bw/2):
                        tshots +=1
                ...
                restart = 1
                   ...
                   if mousey < (y*bh) + 10:
                       fps = int(((mousex-preview_width-bw) / bw) * (video_limits[((y-1)*3) + 2] - video_limits[((y-1)*3) + 1]))
                       fps = min(fps,vfps)
            ...
            elif (g == 32 or g == 33) and awb == 0 :
                   if mousey < (y*bh) + 10:
                       red = (((mousex-preview_width) / bw) * (still_limits[((y-1)*3) + 2] - still_limits[((y-1)*3) + 1])) 
                   elif mousey < (y*bh) + (bh/1.2):
                       if mousex < preview_width + (bw/2):
                           red -=0.1
            ...
            elif (g == 28 or g == 29) and awb == 0:
                   if mousey < (y*bh) + 10:
                       blue = (((mousex-preview_width) / bw) * (still_limits[((y-1)*3) + 2] - still_limits[((y-1)*3) + 1])) 
                   elif mousey < (y*bh) + (bh/1.2):
                       if mousex < preview_width + (bw/2):
                           blue -=0.1
                ...
                if g == 45:
                    effect +=1
                    if effect > len(effects) - 1:
                        effect = len(effects) - 1
                else:
                    effect -=1
                    if effect < 0:
                        effect = 0
                ...
                restart = 1
                   ...
                   # save config
                   text(1,11,3,0,1,"Save Config",fv,7)
                   config[0] = mode
                   config[1] = speed
                   config[2] = ISO
                   config[3] = brightness
