...
vparas =     [vlen,1,9999,fps,1,40,vformat,0,5,a_video,0,1,tduration,1,9999,tinterval,1,9999,tshots,1,999]
pygame.init()
if frame == 0:
...
def draw_bar(col,row,color,value):
    ...
    pygame.draw.rect(windowSurfaceObj,lgrnColor,Rect(cwidth + col*bw,row * bh,bw-1,10))
    ...
    j = min(j,bw-5)
    #print (j,value,parameters[((row-1)*3) + 2],parameters[((row-1)*3) + 1]) 
    ...
    if buttonx[0] != 0 :
        ...
        if mousex > cwidth:
            ...
            if g == 9 :
                ...
                tinterval +=1
                tinterval = min(tinterval,vparas[17])
                if mode == 0 and speed < 6000001:
                    tinterval = max(tinterval,int((speed/1000000) * 6.33))
                if mode == 0 and speed > 6000000:
                    tinterval = max(tinterval,int((speed/1000000)))
                ...
                tinterval -=1
                tinterval = max(tinterval,vparas[16])
                #if mode == 0 and speed < 6000001:
                #    tinterval = max(tinterval,int((speed/1000000) * 6.33))
                #if mode == 0 and speed > 6000000:
                #    tinterval = max(tinterval,int((speed/1000000)))
                ...
                tduration +=1
                tduration = min(tduration,vparas[14])
                text(1,8,3,1,1,str(tduration),fv,12)
                tshots = int(tduration / tinterval)
                text(1,10,3,1,1,str(tshots),fv,12)
            elif g == 34 :
                tduration -=1
                tduration = max(tduration,vparas[13])
                text(1,8,3,1,1,str(tduration),fv,12)
                tshots = int(tduration / tinterval)
                text(1,10,3,1,1,str(tshots),fv,12)
                ...
                tshots +=1
                tshots = min(tshots,vparas[20])
                text(1,10,3,1,1,str(tshots),fv,12)
                tduration = tinterval * tshots
                text(1,8,3,1,1,str(tduration),fv,12)
            elif g == 42 :
                tshots -=1
                tshots = max(tshots,vparas[19])
                text(1,10,3,1,1,str(tshots),fv,12)
                tduration = tinterval * tshots
                text(1,8,3,1,1,str(tduration),fv,12)
